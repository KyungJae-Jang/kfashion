...
```
 ...
 - Google SMTP를 이용한 회원 가입 인증과 패스워드 찾기
 - 인증 이메일 재전송 기능
 - 게시판에 텍스트와 이미지 파일로 게시글 작성
 - 게시글에 댓글과 대댓글 작성
 - 키워드로 게시글 검색 기능
 - 회원 정보 확인
 - 회원 정보 변경(닉네임, 패스워드)
 - 회원 탈퇴(탈퇴 시 게시글 및 댓글도 자동 삭제)
 - 회원 본인이 작성한 게시글과 댓글 목록 열람
 - 이메일 미인증시 회원 가입 날짜기준 2주 뒤 회원 정보 자동 삭제
```
## 4. 프로젝트 중 어려웠던 점
```
 - 멘토의 부재로 인해 문제가 발생했을 때 해결하는 시간이 오래 걸렸고, 문제에 대한 해결책이 맞는지 확신이 없었습니다.
 ...
 - Javascript의 실력 부족으로 스크립트 작성하는데 많은 시간이 소요되었습니다.
 - QA 인원의 부재로 버그 발견이 힘들어 예외 처리가 허술하게 되었습니다.
```
## 5. 프로젝트를 개선 해야할 점
```
 - 테스트 코드 학습을 통해서 각 단위별로 테스트 코드를 작성해서 프로젝트의 완성도를 높이고 싶습니다.  
 - UI 직관성을 높여 사용자가 보기 편하도록 개선
 - 전체적인 코드를 리팩토링하여 모듈화하도록 개선
```
## 6. 앞으로 추가하고 싶은 기능
```
 - 게시글과 댓글에 대한 알람 기능
